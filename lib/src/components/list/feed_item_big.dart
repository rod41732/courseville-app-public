...
class FeedItemList extends StatefulWidget {
  ...
  final Item item;
  FeedItemList({Key key, this.item}) :
    _title = item.title,
  ...
  @override
  _FeedItemListState createState() => _FeedItemListState(item.readFlag == 0);
}
...
class _FeedItemListState extends State<FeedItemList> {
  ...
  _FeedItemListState(bool highlight) {
  ...
  Widget build(BuildContext context) {
    return ListEntryBase(
      ...
      body: Column(
        ...
        children: <Widget>[
          Text(widget._title, style: Theme.of(context).textTheme.subtitle.apply(fontSizeFactor: 1.2), softWrap: true, maxLines: 2),
          Text(widget._content, style: Theme.of(context).textTheme.caption, maxLines: 4),
        ],
      ),
